...
const mockedLoadJsonData = loadJsonData as jest.MockedFunction<typeof loadJsonData>;
...
describe("CompanyRepositoryImpl", () => {
    ...
    it("should load companies correctly from JSON", () => {
        const jsonData = [
            { id: "1", cuit: "12345678901", name: "Test Company", type: "PYME", accounts: ["12345678910"] },
        ];
        ...
        expect(company.accounts.map(acc => acc.getValue())).toEqual(["12345678910"]);
    });

    it("should handle errors in JSON loading and set companies to empty array", () => {
        mockedLoadJsonData.mockImplementation(() => { throw new Error("File not found"); });
    ...
    it("should find companies by adhesion date range", async () => {
        ...
        const result = await repo.findCompaniesByAdhesionDateRange(yesterday, today);
