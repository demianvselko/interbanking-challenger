...
describe("CreateCompanyUseCase", () => {
    ...
    it("should create a company successfully", async () => {
        ...
        jest.spyOn(CuitVO, "create").mockReturnValue(Result.ok({ getValue: () => request.cuit } as CuitVO));
        jest.spyOn(CompanyNameVO, "create").mockReturnValue(Result.ok({ getValue: () => request.name } as CompanyNameVO));
        jest.spyOn(CompanyTypeVO, "create").mockReturnValue(Result.ok({ getValue: () => request.type } as CompanyTypeVO));
        jest.spyOn(AccountNumberVO, "create").mockImplementation(acc =>
    ...
    it("should fail if CuitVO is invalid", async () => {
        const request = { cuit: "invalid", name: "Test", type: "PYME", accounts: [] };
    ...
    it("should fail if CompanyNameVO is invalid", async () => {
        const request = { cuit: "12345678901", name: "", type: "PYME", accounts: [] };
        jest.spyOn(CuitVO, "create").mockReturnValue(Result.ok({ getValue: () => request.cuit } as CuitVO));
        jest.spyOn(CompanyNameVO, "create").mockReturnValue(Result.fail("Invalid name"));
    ...
    it("should fail if CompanyTypeVO is invalid", async () => {
        const request = { cuit: "12345678901", name: "Test", type: "INVALID", accounts: [] };
        jest.spyOn(CuitVO, "create").mockReturnValue(Result.ok({ getValue: () => request.cuit } as CuitVO));
        jest.spyOn(CompanyNameVO, "create").mockReturnValue(Result.ok({ getValue: () => request.name } as CompanyNameVO));
        jest.spyOn(CompanyTypeVO, "create").mockReturnValue(Result.fail("Invalid type"));
    ...
    it("should fail if any AccountNumberVO is invalid", async () => {
        const request = { cuit: "12345678901", name: "Test", type: "PYME", accounts: ["111-222"] };
        jest.spyOn(CuitVO, "create").mockReturnValue(Result.ok({ getValue: () => request.cuit } as CuitVO));
        jest.spyOn(CompanyNameVO, "create").mockReturnValue(Result.ok({ getValue: () => request.name } as CompanyNameVO));
        jest.spyOn(CompanyTypeVO, "create").mockReturnValue(Result.ok({ getValue: () => request.type } as CompanyTypeVO));
        jest.spyOn(AccountNumberVO, "create").mockReturnValue(Result.fail("Invalid account"));
