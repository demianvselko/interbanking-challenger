...
describe("FindCompaniesByAdhesionUseCase", () => {
    ...
    it("should return companies for last month by default", async () => {
        const mockCompanies: Company[] = [
            { name: { getValue: () => "Company A" }, accounts: [], type: "PYME" } as unknown as Company,
            { name: { getValue: () => "Company B" }, accounts: [], type: "GRANDE" } as unknown as Company,
        ];

        (repo.findCompaniesByAdhesionDateRange as jest.Mock).mockResolvedValue(mockCompanies);
        ...
        const startDate = (repo.findCompaniesByAdhesionDateRange as jest.Mock).mock.calls[0][0];
        const endDate = (repo.findCompaniesByAdhesionDateRange as jest.Mock).mock.calls[0][1];
    ...
    it("should return all companies if lastMonth=false", async () => {
        const mockCompanies: Company[] = [
            { name: { getValue: () => "Company X" }, accounts: [], type: "PYME" } as unknown as Company
        ];
    ...
    it("should fail if repository throws an error", async () => {
        (repo.findCompaniesByAdhesionDateRange as jest.Mock).mockRejectedValue(new Error("Failed to fetch companies by adhesion date"));
