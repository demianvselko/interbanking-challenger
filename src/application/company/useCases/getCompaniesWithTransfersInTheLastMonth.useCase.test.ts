...
describe("FindCompaniesWithTransfersUseCase", () => {
    ...
    it("should return companies that have transfers in last month by default", async () => {
        ...
        const mockCompanies: Company[] = [
            { name: { getValue: () => "Company 1" }, accounts: [], type: "PYME" } as unknown as Company,
            { name: { getValue: () => "Company 2" }, accounts: [], type: "GRANDE" } as unknown as Company,
        ];

        (transferRepo.findByDateRange as jest.Mock).mockResolvedValue(mockTransfers);
        ...
        if (result.ok) {
            ...
            expect(result.value.map(c => c.name.getValue())).toEqual(["Company 1", "Company 2"]);
        }
    ...
    it("should fail if transfer repository throws", async () => {
        (transferRepo.findByDateRange as jest.Mock).mockRejectedValue(new Error("Failed to fetch"));
    ...
    it("should fail if company repository throws", async () => {
        ...
        (transferRepo.findByDateRange as jest.Mock).mockResolvedValue(mockTransfers);
        (companyRepo.findByIds as jest.Mock).mockRejectedValue(new Error("Failed to fetch"));
