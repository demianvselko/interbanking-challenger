...
describe("createCompanyHandler", () => {
    const MockedUseCase = CreateCompanyUseCase as jest.MockedClass<typeof CreateCompanyUseCase>;
    ...
    it("should return 201 when company is created successfully", async () => {
        ...
        MockedUseCase.prototype.execute = jest.fn().mockResolvedValue(Result.ok(fakeCompany));
    ...
    it("should return 400 when use case fails validation", async () => {
        MockedUseCase.prototype.execute = jest.fn().mockResolvedValue(Result.fail("Invalid CUIT"));
